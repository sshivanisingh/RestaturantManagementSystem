...
import {
  Navigation2, Phone, Package, Loader2, MapPin,
  AlertCircle, WifiOff, ChevronDown, ChevronUp,
  Copy, Volume2, CheckCircle2, Clock, X,
} from "lucide-react"
...
interface EtaInfo  { durationText: string; distanceText: string; durationSeconds: number }
interface StepInfo { instruction: string; distance: string; maneuver: string; startLat: number; startLng: number }
...
function loadGMaps(): Promise<void> {
  ...
  loadPromise = new Promise((ok, fail) => {
    ...
    s.async = true; s.onload = () => ok(); s.onerror = () => fail(new Error("Maps load error"))
    document.head.appendChild(s)
  })
  return loadPromise
}

function geocodeAddr(address: string): Promise<google.maps.LatLngLiteral | null> {
...
function computeHeading(from: { lat: number; lng: number }, to: { lat: number; lng: number }): number {
...
const MAP_STYLE: google.maps.MapTypeStyle[] = [
  ...
  { featureType: "road.highway",        elementType: "geometry.stroke", stylers: [{ color: "#ddd" }] },
  ...
  { featureType: "water",               elementType: "labels.text.fill",stylers: [{ color: "#9ab" }] },
]

// ── Navigation arrow overlay ───────────────────────────────────────
function makeNavArrow(G: any, lat: number, lng: number, heading: number, map: any) {
  if (!document.getElementById("_nav_css")) {
    const s = document.createElement("style"); s.id = "_nav_css"
    s.textContent = "@keyframes navRing{0%{transform:scale(.4);opacity:.5}100%{transform:scale(2.2);opacity:0}}"
    document.head.appendChild(s)
  }

  class NavOverlay extends G.OverlayView {
    _pos: any; _div: HTMLDivElement | null = null; _inner: HTMLElement | null = null
    constructor() { super(); this._pos = new G.LatLng(lat, lng) }

    onAdd() {
      const d = document.createElement("div")
      d.style.cssText = "position:absolute;pointer-events:none;user-select:none;"
      d.innerHTML = `
      ...
      this._div = d; this._inner = d.querySelector("#_nav_inner")
    ...
    draw() {
      ...
      this._div.style.left = pt.x + "px"; this._div.style.top = pt.y + "px"
    }

    onRemove() { this._div?.parentNode?.removeChild(this._div); this._div = null }
    setHeading(deg: number) { if (this._inner) this._inner.style.transform = `rotate(${deg}deg)` }

    moveTo(toLat: number, toLng: number) {
      const fLat = this._pos.lat(), fLng = this._pos.lng()
      const t0 = performance.now(), dur = 1200
      const step = (now: number) => {
        const p = Math.min((now - t0) / dur, 1)
        const ep = p < 0.5 ? 2 * p * p : -1 + (4 - 2 * p) * p
        this._pos = new G.LatLng(fLat + (toLat - fLat) * ep, fLng + (toLng - fLng) * ep)
        this.draw()
  ...
  const o = new NavOverlay(); o.setMap(map); return o
}

// ── Destination pin ────────────────────────────────────────────────
const destIconUrl = () => `data:image/svg+xml;utf8,${encodeURIComponent(`
...
function SlideToConfirm({ onConfirm, isLoading }: { onConfirm: () => void; isLoading?: boolean }) {
  ...
  const onStart = (cx: number) => {
    ...
    dragging.current = true; startX.current = cx - x
  }
  const onMove = (cx: number) => {
    if (!dragging.current) return
    const nx = Math.max(0, Math.min(cx - startX.current, getMax()))
    setX(nx)
    if (nx >= getMax() - 4) { dragging.current = false; setDone(true); onConfirm() }
  }
  const onEnd = () => { if (!done) { dragging.current = false; setX(0) } }
  ...
  return (
    ...
    >
      <span className="absolute inset-0 flex items-center justify-center text-white/90 text-sm font-semibold tracking-wide pointer-events-none"
      ...
      <div className="absolute top-1 left-1 bottom-1 w-14 rounded-xl bg-white shadow-lg flex items-center justify-center z-10"
        style={{ transform: `translateX(${x}px)`, transition: dragging.current ? "none" : "transform 0.25s ease", cursor: done ? "default" : "grab" }}
        onMouseDown={e => { e.preventDefault(); onStart(e.clientX) }}
        onTouchStart={e => onStart(e.touches[0].clientX)}
      >
        {isLoading ? <Loader2 className="w-5 h-5 text-green-700 animate-spin" />
          : done    ? <CheckCircle2 className="w-5 h-5 text-green-700" />
          :           <span className="text-green-700 font-bold text-xl select-none">»</span>}
      </div>
    </div>
  )
}
...
export default function DeliveryMap({
  ...
  totalItems, totalAmount, deliveryStatus,
  viewAs = "driver",
  onClose, onMarkDelivered, isMarkingDelivered,
}: DeliveryMapProps) {
  ...
  type MapStatus = "loading" | "gps_wait" | "geocoding" | "routing" | "ready" | "error"
  ...
  const deliveryTime = eta
    ? new Date(Date.now() + eta.durationSeconds * 1000)
  ...
  // ── Draw route ─────────────────────────────────────────────────
  const drawRoute = useCallback((fLat: number, fLng: number, tLat: number, tLng: number, map: any) => {
    const G = (window as any).google.maps
    polylinesRef.current.forEach(p => p.setMap(null)); polylinesRef.current = []

    new G.DirectionsService().route(
      { origin: { lat: fLat, lng: fLng }, destination: { lat: tLat, lng: tLng }, travelMode: G.TravelMode.DRIVING },
      (result: any, s: string) => {
        ...
        if (leg) setEta({ durationText: leg.duration.text, distanceText: leg.distance.text, durationSeconds: leg.duration.value })

        polylinesRef.current = [
          new G.Polyline({ path, map, strokeColor: "white",    strokeWeight: 14, strokeOpacity: 0.8, zIndex: 1 }),
          new G.Polyline({ path, map, strokeColor: "#2196F3",  strokeWeight: 6,  strokeOpacity: 1,   zIndex: 2 }),
        ]
        setStatus("ready")
      }
    )
  ...
  // ── Init map ───────────────────────────────────────────────────
  const initMap = useCallback((dLat: number, dLng: number, destination: google.maps.LatLngLiteral) => {
    if (!divRef.current) return
    const G = (window as any).google.maps
    if (mapRef.current) { mapRef.current.unbindAll(); mapRef.current = null }

    const map = new G.Map(divRef.current, {
      center: { lat: dLat, lng: dLng }, zoom: 14,
      disableDefaultUI: true, gestureHandling: "greedy", styles: MAP_STYLE,
    })
    mapRef.current = map

    driverRef.current = makeNavArrow(G, dLat, dLng, headingRef.current, map)
    new G.Marker({ position: destination, map, icon: { url: destIconUrl(), scaledSize: new G.Size(52, 66), anchor: new G.Point(26, 66) } })

    const bounds = new G.LatLngBounds()
    bounds.extend({ lat: dLat, lng: dLng }); bounds.extend(destination)
  ...
  // ── Boot ───────────────────────────────────────────────────────
  useEffect(() => {
    if (!KEY) { setErrMsg("NEXT_PUBLIC_GOOGLE_MAPS_KEY missing"); setStatus("error"); return }
    if (!hasGPS) { setStatus("gps_wait"); return }

    const boot = async () => {
      try {
        setStatus("loading"); await loadGMaps()
        let destination = dest
        if (!destination) {
          setStatus("geocoding")
          destination = await geocodeAddr(customerAddress)
          if (!destination) { setErrMsg(`Address not found: "${customerAddress}"`); setStatus("error"); return }
          setDest(destination)
        }
        initMap(driverLat, driverLng, destination)
      } catch (e: any) { setErrMsg(e?.message ?? "Map failed"); setStatus("error") }
    }
    boot()
    return () => {
  ...
  // ── Live driver update ─────────────────────────────────────────
  useEffect(() => {
    if (!driverRef.current || !dest || !mapRef.current || status !== "ready") return
    if (prevLL.current.lat === driverLat && prevLL.current.lng === driverLng) return

    const h = computeHeading(prevLL.current, { lat: driverLat, lng: driverLng })
    headingRef.current = h; driverRef.current.setHeading(h); driverRef.current.moveTo(driverLat, driverLng)
    prevLL.current = { lat: driverLat, lng: driverLng }

    if (steps.length > 0 && stepIdx < steps.length - 1) {
      const next = steps[stepIdx + 1]
      if (Math.sqrt((next.startLat - driverLat) ** 2 + (next.startLng - driverLng) ** 2) * 111000 < 60)
        setStepIdx(i => i + 1)
    }
  ...
  const reCenter = () => {
    ...
    b.extend({ lat: driverLat, lng: driverLng }); b.extend(dest)
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-shrink-0">
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-gray-400">
            {deliveryStatus === "out_for_delivery" ? "🔴 LIVE" : "Active Delivery"}
          </p>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-orange-50 text-orange-600 border border-orange-100">
            {deliveryStatus === "out_for_delivery" ? "🛵 On the way" : deliveryStatus.replace(/_/g, " ")}
          </span>
          {onClose && (
      ...
      {eta && (
        <div className="grid grid-cols-3 gap-2 px-4 py-3 border-b border-gray-100 flex-shrink-0">
          {[
            { icon: <MapPin   className="w-3.5 h-3.5 text-blue-500"  />, label: "Distance", val: eta.distanceText   },
            { icon: <Clock    className="w-3.5 h-3.5 text-orange-500"/>, label: "ETA",      val: eta.durationText   },
            { icon: <CheckCircle2 className="w-3.5 h-3.5 text-green-500"/>, label: "Arrival", val: deliveryTime    },
          ].map(s => (
            <div key={s.label} className="rounded-xl p-2.5 text-center" style={{ background: "#f8fafc", border: "1px solid #e8f0fe" }}>
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-start gap-3 min-w-0">
            ...
            <div className="min-w-0">
              {customerName && <p className="text-sm font-bold text-gray-800">{customerName}</p>}
              <div className="flex items-start gap-1 mt-0.5">
                <MapPin className="w-3 h-3 text-gray-400 flex-shrink-0 mt-0.5" />
                <p className="text-xs text-gray-500 leading-snug">{customerAddress}</p>
              </div>
            </div>
          </div>
      ...
      <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <button onClick={copyOrderId} className="text-gray-400 hover:text-blue-500">
        ...
        <div className="text-right">
          ...
          <p className="text-sm font-bold text-gray-800 mt-0.5">₹{totalAmount.toLocaleString("en-IN")}</p>
        </div>
      </div>

      {/* Items */}
      <button onClick={() => setItems(v => !v)}
        ...
        {items ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
      </button>
      ...
      {/* Slide to confirm */}
      {viewAs === "driver" && deliveryStatus === "out_for_delivery" && onMarkDelivered && (
        <div className="px-4 pb-6 pt-3 flex-shrink-0">
          <SlideToConfirm onConfirm={onMarkDelivered} isLoading={isMarkingDelivered} />
          <p className="text-center text-xs text-gray-400 mt-2">🔄 Location updates every 10 seconds</p>
        </div>
      )}
    </div>
  )
  ...
  return (
    ...
    */
    <div className="fixed inset-0 z-50 flex flex-col lg:flex-row bg-white">
      ...
      {/* ═══════════════ MAP SECTION ══════════════════════════════ */}
      <div className="relative flex-1 min-h-0 lg:min-h-full overflow-hidden"
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center bg-white/90">
            <div className="flex flex-col items-center gap-4 px-8 text-center max-w-xs">
              ...
              <p className="text-gray-700 font-semibold text-sm">
                {status === "error"     ? errMsg
                 : status === "gps_wait"  ? "Waiting for GPS…"
                 : status === "geocoding" ? "Finding delivery location…"
                 : status === "routing"   ? "Calculating route…"
                 : "Loading map…"}
              </p>
              {status === "gps_wait" && <p className="text-gray-400 text-xs">Please allow location access</p>}
            </div>
          </div>
        )}
        ...
        {status === "ready" && (
          <div className="absolute top-3 left-3 z-10 flex flex-col gap-1.5" style={{ maxWidth: 220 }}>
            {/* Turn instruction pill */}
            <div className="flex items-center gap-2 pl-1.5 pr-3 py-1.5 rounded-2xl bg-white shadow-md"
              style={{ border: "1px solid #e5e7eb" }}>
              <div className="rounded-xl flex items-center justify-center flex-shrink-0 bg-blue-600"
            ...
            <div className="flex items-center rounded-xl bg-white shadow-md overflow-hidden"
              ...
              {[
                ...
                <div key={s.label} className="flex-1 text-center py-1.5"
                  ...
                  <p className="text-gray-800 text-[11px] font-bold leading-tight">{s.val}</p>
        ...
        {status === "ready" && (
          <>
            <div className="absolute right-3 z-10 flex flex-col gap-2" style={{ bottom: 12 }}>
            ...
            <button className="absolute left-3 z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white shadow-lg border border-gray-100"
              style={{ bottom: 12 }}>
              <span className="text-yellow-500 text-xs">⚠</span>
              <span className="text-gray-600 text-xs font-medium">Report Issue</span>
            </button>
          </>
        )}
      </div>
      ...
      {/* Mobile bottom sheet */}
      <div className="lg:hidden bg-white rounded-t-3xl shadow-2xl flex-shrink-0"
        style={{ maxHeight: "45vh", overflowY: "auto", borderTop: "1px solid #e5e7eb" }}>
        {/* Drag handle */}
      ...
      <div className="hidden lg:flex flex-col w-[380px] bg-white border-l border-gray-100"
