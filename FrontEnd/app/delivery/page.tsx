...
export default function DeliveryBoysPage() {
  ...
  const { mutate: registerBoy, isPending: registering } = useRegisterDeliveryBoy({
    onSuccess: () => {
      toast({ title: "Delivery boy registered successfully" })
      setFormOpen(false)
    },
  ...
  const { mutate: approveBoy, isPending: approving } = useApproveDeliveryBoy({
    onSuccess: (_, vars) => {
      const msg =
        vars.isApproved === true  ? "Account approved"     :
        vars.isApproved === false ? "Approval revoked"     :
        vars.isActive   === false ? "Account deactivated"  : "Account activated"
      toast({ title: msg })
      setDetailOpen(false)
    },
    onError: (err) => toast({ title: "Error", description: err.message, variant: "destructive" }),
  ...
  const filtered = boys
    ...
    .filter((b) => {
      if (activeTab === "pending")  return !b.isApproved
  ...
  const openEdit   = (b: DeliveryBoy) => {
    setForm({
      name: b.name, email: b.email, phone: b.phone, password: "",
      vehicleType: b.vehicleType, vehicleNumber: b.vehicleNumber ?? "",
      isApproved: b.isApproved, isActive: b.isActive,
    })
    setFormMode("edit"); setSelected(b); setFormOpen(true)
  }
  ...
  const handleSubmit = () => {
    ...
    if (formMode === "create") {
      ...
      registerBoy({
        ...
        password: form.password, vehicleType: form.vehicleType,
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen bg-gray-50">
        ...
        <main className="flex-1 overflow-auto p-4 md:p-6">
          <div className="max-w-7xl mx-auto space-y-5">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm font-poppins text-gray-500 mt-0.5">
                  Manage delivery partners · {isLoading ? "…" : `${total} registered`}
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <button onClick={openCreate}
                  className="h-9 px-4 rounded-xl bg-gradient-to-r from-brand-primary to-brand-secondary
                    text-white text-sm font-poppins font-medium flex items-center gap-1.5
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              ...
              {/* Table header */}
              <div className="hidden md:grid grid-cols-[2fr_1.5fr_1fr_1fr_1fr_1fr_40px] gap-4 px-5 py-3
                bg-gray-50 border-b border-gray-100 text-xs font-poppins font-semibold text-gray-400 uppercase tracking-wide">
                <span>Delivery Boy</span><span>Vehicle</span><span>Status</span>
                <span>Deliveries</span><span>Rating</span><span>Earnings</span><span />
              </div>
              ...
              {isError && !isLoading && (
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <button onClick={() => refetch()}
                    className="h-8 px-4 rounded-xl border border-gray-200 text-sm font-poppins text-gray-600 hover:bg-gray-50 transition-colors flex items-center gap-1.5">
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-3">
                  ...
                  <div className="text-center">
                    <p className="font-satisfy text-[hsl(var(--brand-primary))] text-lg">
                      {search ? `No results for "${search}"` : "No delivery boys found"}
                    </p>
                    {search && (
                      <button onClick={() => setSearch("")} className="text-xs font-poppins text-orange-500 underline mt-1">
                        Clear search
                      </button>
                    )}
                  </div>
              ...
              {!isLoading && !isError && filtered.map((boy, idx) => (
                <div key={boy._id}
                  className={`grid grid-cols-[2fr_1.5fr_1fr_1fr_1fr_1fr_40px] gap-4 px-5 py-4 items-center
                ...
                >
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="relative flex-shrink-0">
                      <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-primary to-brand-secondary
                        flex items-center justify-center text-white font-satisfy text-base select-none">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-1 mt-0.5">
                        ...
                        <span className="text-xs font-poppins text-gray-400">{boy.phone}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-orange-50 flex items-center justify-center flex-shrink-0">
                      <VehicleIcon type={boy.vehicleType} className="w-4 h-4 text-orange-400" />
                    </div>
                    <div className="min-w-0">
                  ...
                  <div className="flex flex-col gap-1">
                    {!boy.isApproved ? (
                      <StatusBadge color="bg-amber-100 text-amber-700">Pending</StatusBadge>
                    ) : !boy.isActive ? (
                      <StatusBadge color="bg-red-100 text-red-700">Inactive</StatusBadge>
                    ) : boy.isOnline ? (
                      <StatusBadge color="bg-emerald-100 text-emerald-700">Online</StatusBadge>
                    ) : (
                      <StatusBadge color="bg-gray-100 text-gray-500">Offline</StatusBadge>
                    )}
                    {boy.currentOrderId && (
                      <StatusBadge color="bg-blue-100 text-blue-700">On Delivery</StatusBadge>
                    )}
                  </div>
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-sm font-poppins font-semibold text-gray-700">{boy.totalDeliveries}</span>
                  </div>
                    ...
                    {boy.totalRatings > 0 ? (
                      <>
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-sm font-poppins font-semibold text-gray-700">{boy.averageRating.toFixed(1)}</span>
                        </div>
                        <p className="text-xs font-poppins text-gray-400">{boy.totalRatings} reviews</p>
                      </>
                    ) : (
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-sm font-poppins font-semibold text-gray-700">
                      {boy.totalEarnings.toLocaleString("en-IN")}
                    </span>
                  </div>
              ...
              {!isLoading && !isError && totalPages > 1 && (
                <div className="flex items-center justify-between px-5 py-3 border-t border-gray-100 bg-gray-50">
                  ...
                  <div className="flex items-center gap-1">
                    <button disabled={page <= 1 || isFetching} onClick={() => handlePage(page - 1)}
                      className="h-7 w-7 rounded-lg border border-gray-200 flex items-center justify-center
                    ...
                    {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => {
                      ...
                      return (
                        <button key={p} disabled={isFetching} onClick={() => handlePage(p)}
                          className={`h-7 w-7 rounded-lg text-xs font-poppins font-medium transition-all
                            ${p === page
                    ...
                    <button disabled={page >= totalPages || isFetching} onClick={() => handlePage(page + 1)}
                      className="h-7 w-7 rounded-lg border border-gray-200 flex items-center justify-center
                        text-gray-500 hover:bg-gray-100 disabled:opacity-40 transition-colors">
          ...
          <div className="overflow-y-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none]">
            <div className="p-6 space-y-5">
              ...
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-brand-primary to-brand-secondary
                  flex items-center justify-center text-white font-satisfy text-2xl flex-shrink-0">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <StatusBadge color={selected.isActive ? "bg-blue-100 text-blue-700" : "bg-red-100 text-red-700"}>
                  {selected.isActive ? "Active" : "Inactive"}
                </StatusBadge>
                <StatusBadge color={selected.isOnline ? "bg-emerald-100 text-emerald-700" : "bg-gray-100 text-gray-500"}>
                  <StatusDot online={selected.isOnline} />
                  <span className="ml-1">{selected.isOnline ? "Online" : "Offline"}</span>
                </StatusBadge>
                {selected.currentOrderId && (
                  <StatusBadge color="bg-blue-100 text-blue-700">
              ...
              <div className="bg-gray-50 rounded-2xl p-4 space-y-3">
                ...
                {[
                  ...
                  { icon: <VehicleIcon type={selected.vehicleType} className="w-4 h-4" />, label: "Vehicle",
                    value: `${selected.vehicleType.charAt(0).toUpperCase() + selected.vehicleType.slice(1)}${selected.vehicleNumber ? " · " + selected.vehicleNumber : ""}` },
                  { icon: <MapPin className="w-4 h-4" />, label: "Last Seen",
                    value: selected.lastLocationUpdatedAt
                      ? `Updated ${new Date(selected.lastLocationUpdatedAt).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}`
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { label: "Rating",
                    value: selected.totalRatings > 0 ? `${selected.averageRating.toFixed(1)} ★` : "—",
                    sub: selected.totalRatings > 0 ? `${selected.totalRatings} reviews` : "",
                    icon: <Star className="w-4 h-4 text-amber-400" />, bg: "bg-amber-50" },
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <button onClick={() => { setDetailOpen(false); openEdit(selected) }}
                  className="h-9 px-4 rounded-xl border border-gray-200 text-sm font-poppins text-gray-600
                    hover:bg-gray-50 transition-colors">
                  Edit Profile
                </button>
                <button disabled={approving}
                  onClick={() => approveBoy({ deliveryBoyId: selected._id, isApproved: !selected.isApproved })}
                  className={`h-9 px-4 rounded-xl text-sm font-poppins font-medium flex items-center gap-1.5 transition-colors disabled:opacity-60
                    ${selected.isApproved
                ...
                <button disabled={approving}
                  onClick={() => approveBoy({ deliveryBoyId: selected._id, isActive: !selected.isActive })}
                  className={`h-9 px-4 rounded-xl text-sm font-poppins font-medium flex items-center gap-1.5 border transition-colors disabled:opacity-60
                    ${selected.isActive
      ...
      <Modal open={formOpen} onClose={() => setFormOpen(false)}>
        <ModalHeader
          title={formMode === "create" ? "Register Delivery Boy" : "Edit Profile"}
        ...
        <div className="overflow-y-auto [&::-webkit-scrollbar]:hidden [scrollbar-width:none] p-6 space-y-4">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-poppins font-medium text-gray-500">Full Name <span className="text-red-400">*</span></Label>
            <Input name="name" value={form.name} onChange={handleInput} placeholder="e.g. Ravi Kumar" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-poppins font-medium text-gray-500">Email <span className="text-red-400">*</span></Label>
            <Input name="email" type="email" value={form.email} onChange={handleInput}
              placeholder="ravi@example.com" disabled={formMode === "edit"} />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-poppins font-medium text-gray-500">Phone <span className="text-red-400">*</span></Label>
            <Input name="phone" value={form.phone} onChange={handleInput} placeholder="10-digit mobile" maxLength={10} />
          </div>
          {formMode === "create" && (
            <div className="space-y-1.5">
              <Label className="text-xs font-poppins font-medium text-gray-500">Password <span className="text-red-400">*</span></Label>
              <Input name="password" type="password" value={form.password} onChange={handleInput} placeholder="Min. 8 characters" />
            </div>
          )}
          ...
          <div className="space-y-1.5">
            ...
            <select name="vehicleType" value={form.vehicleType} onChange={handleInput}
              className="w-full h-10 px-3 border border-input rounded-md text-sm font-poppins bg-background
          ...
          <div className="space-y-1.5">
            <Label className="text-xs font-poppins font-medium text-gray-500">
              Vehicle Number <span className="text-gray-400 font-normal">(optional)</span>
            </Label>
            <Input name="vehicleNumber" value={form.vehicleNumber} onChange={handleInput}
              placeholder="e.g. DL 4C AB 1234" className="font-mono" />
          </div>
            ...
            <>
              ...
              {[
                ...
                <div key={key} className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                  <div>
                    <p className="text-sm font-poppins font-medium text-gray-700">{label}</p>
                    <p className="text-xs font-poppins text-gray-400">{sub}</p>
                  </div>
          ...
          <div className="flex gap-3 pt-2">
            <button onClick={() => setFormOpen(false)} disabled={registering || updating}
              className="flex-1 h-10 rounded-xl border border-gray-200 text-sm font-poppins text-gray-600
                hover:bg-gray-50 transition-colors disabled:opacity-50">
              Cancel
            </button>
            <button onClick={handleSubmit} disabled={registering || updating}
              className="flex-1 h-10 rounded-xl bg-gradient-to-r from-brand-primary to-brand-secondary
                text-white text-sm font-poppins font-medium hover:opacity-90 transition-opacity
