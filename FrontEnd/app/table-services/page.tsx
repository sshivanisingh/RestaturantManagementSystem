...
// ─── Stat Card ─────────────────────────────────────────────────────────────────
function StatCard({ label, value, color, bg, dotColor }: {
  label: string; value: number; color: string; bg: string; dotColor: string;
}) {
  return (
  ...
  return (
    <button onClick={onClick}
      ...
      <div className="flex items-start justify-between mb-3">
        ...
        <span className={`text-xs font-poppins font-semibold px-2.5 py-1 rounded-full ${cfg.badge}`}>
          {cfg.label}
        </span>
      </div>
      <p className="text-lg font-satisfy text-gray-800 group-hover:text-[hsl(var(--brand-primary))] transition-colors">
        {table.tableLabel}
      </p>
...
function DeleteModal({ open, label, onConfirm, onClose, loading }: {
  open: boolean; label: string; onConfirm: () => void; onClose: () => void; loading: boolean;
}) {
  return (
      ...
      <div className="p-6 text-center space-y-4">
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button onClick={onConfirm} disabled={loading}
            className="flex-1 h-10 rounded-xl bg-red-500 text-white text-sm font-poppins font-medium
...
export default function TableServicesPage() {
  ...
  const handleCreate = () => {
    if (!createLabel.trim()) {
      toast({ title: "Label required", description: "Please enter a table label.", variant: "destructive" })
      return
    }
  ...
  const openEdit = (table: Table) => {
    ...
    setFormLabel(table.tableLabel)
    setEditOpen(true)
  }

  const handleSave = () => {
    if (!selected) return
    if (formLabel.trim() !== selected.tableLabel) {
      updateTable({ id: selected._id, data: { tableLabel: formLabel.trim(), status: formStatus } })
      return
    }
    if (formStatus !== selected.status) {
      updateStatus({ id: selected._id, status: formStatus })
      return
    }
    setEditOpen(false)
  }
      ...
      <div className="max-w-7xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {(["all", "available", "occupied", "reserved"] as const).map((f) => (
            <button key={f} onClick={() => setFilter(f)}
              ...
              {capitalize(f)}
              {f !== "all" && <span className="ml-1 opacity-70">({stats[f as keyof typeof stats]})</span>}
              {f === "all" && <span className="ml-1 opacity-70">({tables.length})</span>}
            </button>
          ))}
        </div>
        ...
        {isError && !isLoading && (
          <div className="flex flex-col items-center justify-center h-48 gap-3">
            ...
            <button onClick={() => refetch()}
              className="h-9 px-4 rounded-xl border border-gray-200 text-sm font-poppins text-gray-600 hover:bg-gray-50 transition-colors">
              Try again
            </button>
          </div>
        )}
        ...
        {!isLoading && !isError && tables.length === 0 && (
          <div className="flex flex-col items-center justify-center h-64 gap-4">
            ...
            <button onClick={() => setCreateOpen(true)}
              className="h-10 px-5 rounded-xl bg-gradient-to-r from-brand-primary to-brand-secondary
                text-white text-sm font-poppins font-medium flex items-center gap-2 hover:opacity-90 transition-opacity">
              <Plus className="w-4 h-4" /> Add Table
            </button>
          </div>
        )}
        ...
        {!isLoading && !isError && filtered.length > 0 && (
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {filtered.map((table) => (
              <TableCard key={table._id} table={table} onClick={() => openEdit(table)} />
            ))}
          </div>
        )}
        ...
        {!isLoading && !isError && tables.length > 0 && filtered.length === 0 && (
          <div className="py-16 text-center">
            <p className="font-poppins text-gray-400 text-sm">No {filter} tables found.</p>
          </div>
        )}
      </div>
        </main>
        ...
        <div className="p-6 space-y-5">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full h-10 px-3 rounded-xl border border-gray-200 bg-gray-50 text-sm font-poppins
                text-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-200 focus:border-orange-300 transition-all"
            />
          </div>
                ...
                return (
                  <button key={s} onClick={() => setFormStatus(s)}
                    className={`h-11 rounded-xl flex flex-col items-center justify-center gap-0.5 border-2 transition-all
                      ${formStatus === s
                        ? "border-[hsl(var(--brand-primary))] bg-orange-50"
                        : "border-gray-100 bg-gray-50 hover:border-gray-200"}`}>
                    <Icon className={`w-4 h-4 ${
                      s === "available" ? "text-green-600"
                      : s === "occupied" ? "text-red-500"
                      : "text-yellow-600"}`} />
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={() => { setDeleteOpen(true) }}
              className="h-10 px-3 rounded-xl border border-red-200 text-red-500 text-sm font-poppins
                hover:bg-red-50 transition-colors flex items-center gap-1.5">
