...
import { Loader2, User, UtensilsCrossed, ShieldCheck, Check, Zap, BarChart3, Rocket } from "lucide-react"

// ─── Types ────────────────────────────────────────────────────────────────────
interface FormData {
  ownerFullName: string; ownerEmail: string; ownerPhone: string; ownerAddress: string
  name: string; restaurantEmail: string; restaurantPhone: string; restaurantAddress: string
  password: string; confirmPassword: string; logo: File | null
}
...
function Field({ label, id, type = "text", value, onChange, placeholder, required = true }: {
  ...
  return (
    <div className="space-y-1.5">
      <Label htmlFor={id} className="text-sm font-medium text-gray-700 font-poppins">
        {label} {required && <span style={{ color: "hsl(var(--brand-primary))" }}>*</span>}
      </Label>
      <Input
...
function OtpModal({ email, onClose, onVerified }: {
  email: string; onClose: () => void; onVerified: () => void
}) {
  ...
  const { mutate: verifyOtp, isPending: verifying } = useVerifyOtp({
    ...
    onError: (err: any) => {
      toast({ title: "Invalid OTP", description: err?.response?.data?.message || "Wrong OTP", variant: "destructive" })
      setShake(true)
      setTimeout(() => { setShake(false); setOtp(Array(6).fill("")); inputRefs.current[0]?.focus() }, 600)
    },
  })

  const { mutate: resendOtp, isPending: resending } = useSendOtp({
    onSuccess: () => {
      toast({ title: "OTP Resent! 📧", description: `New OTP sent to ${email}` })
      setTimer(30); setCanResend(false); setOtp(Array(6).fill(""))
      inputRefs.current[0]?.focus()
    },
    onError: (err: any) =>
      toast({ title: "Failed to resend", description: err?.response?.data?.message || "Try again", variant: "destructive" }),
  })
      ...
      <div className="relative w-full max-w-sm bg-white shadow-xl overflow-hidden
        ...
        <div className="px-7 pt-6 pb-7 space-y-5">
          <div className="text-center">
            ...
            <h3 className="text-xl font-satisfy" style={{ color: "hsl(var(--brand-primary))" }}>
              Verify Your Email
            </h3>
            <p className="text-xs font-poppins text-gray-400 mt-1">We sent a 6-digit code to</p>
            <p className="text-sm font-poppins font-semibold mt-0.5"
              style={{ color: "hsl(var(--brand-dark))" }}>{email}</p>
          </div>
          ...
          <Button onClick={handleVerify} disabled={verifying || filled < 6}
          ...
          <div className="text-center">
            {canResend ? (
              <button type="button" onClick={() => resendOtp({ email })} disabled={resending}
              ...
              <p className="text-xs font-poppins text-gray-400">
                Resend in{" "}
                <span className="font-bold tabular-nums" style={{ color: "hsl(var(--brand-primary))" }}>
                  {timer}s
                </span>
              </p>
            )}
...
export default function RegisterPage() {
  ...
  const [form,          setForm]          = useState<FormData>({
    ...
    name: "", restaurantEmail: "", restaurantPhone: "", restaurantAddress: "",
    password: "", confirmPassword: "", logo: null
  })
  ...
  const { mutate: register, isPending } = useRegister({
    onSuccess: () => toast({ title: "🎉 Account Created!", description: "Welcome aboard!" }),
    onError: (err: any) =>
  ...
  const { mutate: sendOtp, isPending: sendingOtp } = useSendOtp({
    onSuccess: () => {
      toast({ title: "OTP Sent! 📧", description: `Check your inbox: ${form.ownerEmail}` })
      setOtpModal(true)
    },
    onError: (err: any) =>
      toast({ title: "Failed to send OTP", description: err?.response?.data?.message || "Try again", variant: "destructive" }),
  })
  ...
  const handleSendOtp = () => {
    if (!form.ownerEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.ownerEmail)) {
      toast({ title: "Invalid email", description: "Enter a valid email first", variant: "destructive" }); return
    }
  ...
  const handleLogoFile = (file: File) => {
    if (!file.type.startsWith("image/")) {
      toast({ title: "Invalid file", description: "Please upload an image file", variant: "destructive" }); return
    }
  ...
  const validateStep = () => {
    if (step === 1) {
      if (!form.ownerFullName || !form.ownerEmail || !form.ownerPhone || !form.ownerAddress) {
        toast({ title: "Missing fields", description: "Please fill all owner details", variant: "destructive" }); return false
      }
      if (!emailVerified) {
        toast({ title: "Email not verified", description: "Verify your email with OTP to continue", variant: "destructive" }); return false
      }
    }
    if (step === 2) {
      if (!form.name || !form.restaurantEmail || !form.restaurantPhone || !form.restaurantAddress) {
        toast({ title: "Missing fields", description: "Please fill all restaurant details", variant: "destructive" }); return false
      }
    }
    return true
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (form.password !== form.confirmPassword) {
      toast({ title: "Password mismatch", description: "Passwords must match", variant: "destructive" }); return
    }
    if (form.password.length < 8) {
      toast({ title: "Weak password", description: "Minimum 8 characters required", variant: "destructive" }); return
    }
    ...
    <div className="min-h-screen bg-[#fdf6f0] flex items-center justify-center p-4">
      ...
      {otpModal && (
        <OtpModal
          ...
          onVerified={() => { setEmailVerified(true); setOtpModal(false) }}
        />
      )}
      ...
      <div className="w-full max-w-2xl bg-white shadow-md overflow-hidden">
        ...
        <div className="px-8 pt-8 pb-6 text-center border-b border-gray-100">
          ...
          <h1 className="text-2xl font-satisfy" style={{ color: "hsl(var(--brand-primary))" }}>
        ...
        <div className="px-8 py-5 bg-orange-50/40 border-b border-gray-100">
          <div className="flex items-center justify-between relative">
            ...
            <div className="absolute left-8 top-5 h-0.5 transition-all duration-500"
              style={{
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-8 py-6">
            ...
            {step === 1 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h2 className="text-lg font-satisfy" style={{ color: "hsl(var(--brand-dark))" }}>
                    Owner Details
                  </h2>
                </div>

                <Field label="Full Name" id="ownerFullName" value={form.ownerFullName}
                  onChange={set("ownerFullName")} placeholder="Rahul Sharma" />

                {/* Email + OTP */}
                <div className="space-y-1.5">
                  <Label htmlFor="ownerEmail" className="text-sm font-medium text-gray-700 font-poppins">
                    Email <span style={{ color: "hsl(var(--brand-primary))" }}>*</span>
                  </Label>
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      <Input
                        id="ownerEmail" type="email" value={form.ownerEmail} required
                    ...
                    <button type="button"
                      ...
                      className={`h-11 px-4 text-sm rounded-md font-poppins font-semibold whitespace-nowrap
                        ...
                        ${emailVerified
                          ...
                          : "text-white"}`}
                      style={!emailVerified ? { background: "linear-gradient(135deg, hsl(var(--brand-primary)), hsl(var(--brand-secondary)))" } : {}}>
                      {sendingOtp
                        ? <><Loader2 className="w-3.5 h-3.5 animate-spin" />Sending…</>
                        : emailVerified ? <>✓ Verified</> : <>📨 Send OTP</>}
                    </button>
                  </div>
                  <p className={`text-xs font-poppins ${emailVerified ? "text-green-500" : "text-gray-400"}`}>
                    {emailVerified
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Phone" id="ownerPhone" type="tel" value={form.ownerPhone}
                    onChange={set("ownerPhone")} placeholder="9876543210" />
                  <Field label="Address" id="ownerAddress" value={form.ownerAddress}
                    onChange={set("ownerAddress")} placeholder="42, MG Road, Ghaziabad" />
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                ...
                <Field label="Restaurant Name" id="name" value={form.name}
                  onChange={set("name")} placeholder="Spice Garden" />

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <Field label="Restaurant Email" id="restaurantEmail" type="email"
                    value={form.restaurantEmail} onChange={set("restaurantEmail")}
                    placeholder="info@spicegarden.com" />
                  <Field label="Restaurant Phone" id="restaurantPhone" type="tel"
                    value={form.restaurantPhone} onChange={set("restaurantPhone")}
                    placeholder="0120-4567890" />
                </div>

                <Field label="Restaurant Address" id="restaurantAddress"
                  value={form.restaurantAddress} onChange={set("restaurantAddress")}
                    ...
                    {logoPreview ? (
                      <div className="flex items-center gap-4 p-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-poppins font-medium text-gray-700 truncate">{form.logo?.name}</p>
                      ...
                      <div className="flex flex-col items-center py-8 text-center">
                        ...
                        <p className="text-sm font-poppins text-gray-500">
                          Drop logo here or{" "}
                          <span className="underline" style={{ color: "hsl(var(--brand-primary))" }}>browse</span>
                        </p>
                        <p className="text-xs text-gray-400 font-poppins mt-1">PNG, JPG, WebP — max 5MB</p>
                      </div>
                    )}
                  </div>
            ...
            {step === 3 && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300">
                      ...
                      {[1, 2, 3, 4].map((i) => {
                        ...
                        return (
                          <div key={i} className={`h-1.5 flex-1 transition-all duration-300 ${
                            i <= s
                              ? s <= 1 ? "bg-red-400" : s <= 2 ? "bg-yellow-400" : s <= 3 ? "bg-blue-400" : "bg-green-400"
                              : "bg-gray-200"
                          }`} />
                        )
                      })}
                ...
                {form.confirmPassword && (
                  <p className={`text-xs font-poppins ${form.password === form.confirmPassword ? "text-green-500" : "text-red-400"}`}>
                ...
                <div className="border border-orange-100 bg-orange-50/50 p-4 space-y-2">
                  <p className="text-xs font-poppins font-semibold text-gray-700 mb-2">📋 Summary</p>
                  {[
                    { label: "Owner",      value: form.ownerFullName },
                    { label: "Email",      value: `${form.ownerEmail} ✓` },
                    { label: "Restaurant", value: form.name },
                    { label: "Logo",       value: form.logo ? "✅ " + form.logo.name : "Not uploaded" },
                  ].map(({ label, value }) => (
          ...
          <div className="px-8 pb-8 flex items-center gap-3">
            ...
            {step < 3 ? (
              ...
              <Button type="submit" disabled={isPending || form.password !== form.confirmPassword}
