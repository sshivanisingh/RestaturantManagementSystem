...
function ForgotPasswordModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const { mutate: forgotPassword, isPending } = useForgotPassword({
    onSuccess: () => {
      setSent(true)
      toast({ title: "Reset link sent! 📧", description: `Check your inbox: ${email}` })
    },
    onError: (err: any) =>
      toast({
        title      : "Failed to send reset link",
        description: err?.response?.data?.message || "Unable to connect to server. Please try again.",
        variant    : "destructive",
      }),
  })
  ...
  const handleOpenChange = (val: boolean) => {
    if (!val) { onClose(); setTimeout(() => { setEmail(""); setSent(false) }, 300) }
  }
          ...
          {!sent ? (
            <>
              <DialogHeader className="text-center items-center mb-5">
                ...
                <DialogTitle className="text-xl font-satisfy"
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="forgot-email" className="text-sm font-medium font-poppins text-gray-700">
                    Owner Email <span style={{ color: "hsl(var(--brand-primary))" }}>*</span>
                  </Label>
                  <Input
                    id="forgot-email" type="email" value={email} required
            ...
            <div className="text-center space-y-4 py-2">
              ...
              <div>
                <h3 className="text-lg font-satisfy" style={{ color: "hsl(var(--brand-primary))" }}>
                  Check Your Inbox!
                </h3>
                <p className="text-xs font-poppins text-gray-500 mt-1">Reset link sent to</p>
...
function LoginForm() {
    ...
    return (
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-gradient-to-br from-orange-50 via-white to-pink-50">
        ...
        <div className="relative flex items-center justify-center mb-8">
          <span className="absolute inline-flex h-28 w-28 rounded-full opacity-20 animate-ping"
            style={{ background: "hsl(var(--brand-primary))" }} />
          <span className="absolute inline-flex h-20 w-20 rounded-full opacity-30 animate-ping [animation-delay:0.3s]"
        ...
        <p className="text-sm font-poppins text-gray-400 mb-8">Setting up your dashboard…</p>
  ...
  return (
    <div className="min-h-screen bg-[#fdf6f0] flex items-center justify-center p-4">
      <ForgotPasswordModal open={forgotModal} onClose={() => setForgotModal(false)} />
      ...
      <div className="w-full max-w-sm bg-white shadow-md p-8 space-y-6">
        ...
        <div className="flex flex-col items-center gap-2">
          ...
          <h1 className="text-xl font-semibold font-poppins text-gray-800">Sign in</h1>
          <p className="text-sm font-poppins text-gray-400">Sign in to your restaurant dashboard</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="email" className="text-sm font-poppins text-gray-600">Email address</Label>
            <Input
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <Label htmlFor="password" className="text-sm font-poppins text-gray-600">Password</Label>
        ...
        <div className="relative">
          ...
          <div className="relative flex justify-center text-xs">
            <span className="bg-white px-3 text-gray-400 font-poppins">or continue with</span>
          </div>
        </div>
        ...
        <div className="space-y-2 text-center">
          <p className="text-xs font-poppins text-gray-400">
            Delivery partner?{" "}
            <Link href="/deliveryboy/login" className="hover:underline"
              style={{ color: "hsl(var(--brand-primary))" }}>
              Delivery Portal
            </Link>
          </p>
